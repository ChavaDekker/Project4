            ...
            foreach (Duodata<string, int> i in aDuodataList)
            {
                ...
                for (int x = 0; x < GWidth; x++)
                {
                    for (int y = 0; y < GHeight; y++)
                    {
                        ...
                        if ((x < percentage * GWidth) && y > iteration * (GBarSize + 10) && y < ((iteration + 1) * (GBarSize + 10) - 10)) //if pixel on [x,y] falls within drawable bounds of the bar in the current iteration
                        {
